<template>
    <div class="pager-wrapper" v-if="totalPage > 0">
        <div class="pager-pages">
            <a href="#" v-show="currentPage > 1 && showPrev" @click.prevent="go(currentPage - 1)">上一頁</a>
            <a href="#" :class="{active: currentPage == 1 ? true : false}" @click.prevent="go(1)">1</a>
            <strong v-show="pages[0] > 2">...</strong>
            <a href="#" v-for="(page,index) in pages" :key="index" :class="{active: currentPage == page ? true : false}" @click.prevent="go(page)">{{page}}</a>
            <strong v-show="pages[pages.length-1] < totalPage - 1">...</strong>
            <a href="#" v-if="totalPage > 1" :class="{active: currentPage == totalPage ? true : false}" @click.prevent="go(totalPage)">{{totalPage}}</a>
            <a href="#" v-show="currentPage < totalPage && showNext" @click.prevent="go(currentPage + 1)">下一頁</a>
        </div>
        <div v-if="showJump" v-show="totalPage > 1" class="pager-jump">
            ...
            <span>頁</span>
            <a href="#" @click.prevent="go(jumpPage)">Go</a>
        </div>    
    </div>
</template>
    ...
    export default {
        ...
        created() {
            ...
            if (this.mode == 'params' && !this.routeName) {
                throw 'need a href="#"route name when choose params mode in pager component'
            }
        },
        computed: {
            pages() {
